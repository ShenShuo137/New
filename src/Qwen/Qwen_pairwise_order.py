...
def evaluate_order_verification_task(
    test_json_path,
    model_base_path,
    tsv_output_dir,
    lora_ckpt_path=None,
    ...
    """
    评估模型在“时间顺序验证”(True/False)任务上的表现。
    - 自动处理JSON中提供的任意数量图片。
    - 解析模型的输出以确定是"True"还是"False"。
    - 动态报告True/False各自的准确率。
    """
    # === 1. 加载模型 & 处理器 (无变化) ===
    print("🚀 开始加载模型和处理器...")
    ...
    for entry in tqdm(test_data, desc="🧠 Evaluating Order Verification"):
        ...
        # --- MODIFICATION 1: 新的答案解析逻辑，用于"True"或"False" ---
        predicted_answer = ""
        output_lower = model_output.lower()
        # 检查是否包含 "true" 或 "false" 关键字
        # 这个逻辑可以处理 "The answer is True." 或 "It's false." 等自然语言回答
        if "true" in output_lower:
            predicted_answer = "True"
        elif "false" in output_lower:
            predicted_answer = "False"
        # 如果模型输出了无法识别的内容，predicted_answer将保持为空字符串，并在评估时被判为错误
    ...
    tsv_save_path = os.path.join(tsv_output_dir, f"order_verification_eval_result_{datetime.now().strftime('%Y%m%d_%H%M%S')}.tsv")
    ...
    # --- MODIFICATION 3: 动态报告 (这里会自动变为对"True"和"False"的报告) ---
    option_accuracies = {}
    ...
    for answer in available_answers:
        ...
        # 报告会显示 "Ground Truth 'True': ..." 和 "Ground Truth 'False': ..."
        print(f"  - Ground Truth '{answer}': {stats['accuracy']:.4f} (预测正确 {stats['correct']} / 总数 {stats['count']})")
    print("="*50 + "\n")
...
if __name__ == "__main__":
    ...
    # --- 请根据你的实际情况修改以下路径 ---
    # 这是你的顺序验证任务的测试集JSON文件
    test_json_path = "/data/ss/benchmark/json/TOU/task_datasets/pairwise_order_swift_test.json" # <--- 在这里填入正确的路径
    ...
    if os.path.exists(test_json_path):
        evaluate_order_verification_task(
            test_json_path, 
            model_base_path, 
            tsv_output_dir,
