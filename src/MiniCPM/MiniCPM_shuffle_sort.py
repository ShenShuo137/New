...
def evaluate_image_reordering_task_minicpm(
    test_json_path,
    model_base_path,
    tsv_output_dir,
    lora_ckpt_path=None,
    ...
    """
    评估 MiniCPM-V 模型在“图像乱序重排”任务上的表现，使用PNR作为核心指标。
    - 支持可选的 LoRA 权重加载。
    """
    ...
    for entry in tqdm(test_data, desc="🧠 Evaluating Image Reordering (MiniCPM-V)"):
    ...
    tsv_save_path = os.path.join(tsv_output_dir, f"MiniCPM_image_reordering_eval_{datetime.now().strftime('%Y%m%d_%H%M%S')}.tsv")
...
if __name__ == "__main__":
    ...
    if os.path.exists(test_json_path):
        evaluate_image_reordering_task_minicpm(
            test_json_path, 
            MODEL_BASE_PATH, 
            TSV_OUTPUT_DIR,
