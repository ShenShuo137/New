...
def evaluate_verification_task_minicpm(
    task_name: str,
    test_json_path: str,
    model_base_path: str,
    tsv_output_dir: str,
    ...
    """
    一个通用的函数，用于评估 MiniCPM-V 模型在任何二元判断（True/False）任务上的表现。
    - 支持可选的 LoRA 权重加载。
    """
    print(f"\n{'='*20} Starting MiniCPM-V Verification Task: {task_name.upper()} {'='*20}")
    ...
    for entry in tqdm(test_data, desc=f"🧠 Evaluating {task_name} (MiniCPM-V)"):
        ...
        try:
            with torch.no_grad():
                response = model.chat(msgs=msgs, tokenizer=tokenizer, max_new_tokens=64)
    ...
    for option in possible_options:
        ...
        print(f"  - Ground Truth '{option}': {accuracy:.4f} (预测正确 {correct_for_option} / 总数 {count})")
    print("="*50 + "\n")
...
if __name__ == "__main__":
    ...
    VERIFICATION_TASKS = {
        'is_true_order': '/data/ss/benchmark/json/TOU/task_datasets/is_true_order_swift_test.json',
        'pairwise_order': '/data/ss/benchmark/json/TOU/task_datasets/pairwise_order_swift_test.json',
    }

    # --- 循环运行所有定义的任务 ---
    for task_name, json_path in VERIFICATION_TASKS.items():
        if os.path.exists(json_path):
            evaluate_verification_task_minicpm(
                task_name=task_name,
