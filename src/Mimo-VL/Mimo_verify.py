...
def evaluate_verification_task_mimo_vl(
    task_name: str,
    test_json_path: str,
    model_base_path: str,
    tsv_output_dir: str,
    ...
    """
    一个通用的函数，用于评估 MiMo-VL 模型在任何二元判断（True/False）任务上的表现。
    - 支持可选的 LoRA 权重加载。
    """
    print(f"\n{'='*20} Starting MiMo-VL Verification Task: {task_name.upper()} {'='*20}")
...
if __name__ == "__main__":
    ...
    for task_name, json_path in VERIFICATION_TASKS.items():
        if os.path.exists(json_path):
            evaluate_verification_task_mimo_vl(
                task_name=task_name,
